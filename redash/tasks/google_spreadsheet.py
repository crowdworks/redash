...
def export_google_spreadsheet(query_id):
    ...
    if worksheet is None:
        for ws in spreadsheet.worksheets():
            if ws.title.startswith("redash-export:"):
                worksheet = ws
                break
    if worksheet is None:
        worksheet = spreadsheet.get_worksheet(0)
        worksheet.update_title("redash-export")
    ...
    worksheet.update_title("redash-export: execute={},update={}".format(
        query_result.retrieved_at.strftime('%Y/%m/%d %H:%M'),
